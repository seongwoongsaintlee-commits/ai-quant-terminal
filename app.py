...
# ==========================================
# 1. 실시간 스크리닝 - 안전한 3단 폴백 구조
# ==========================================

def _fetch_yahoo_predefined(scr_id, market_label, max_stocks=300):
    """Yahoo Finance predefined screener - 인증 불필요"""
    rows = []
    for offset in range(0, max_stocks, 100):
        try:
            res = requests.get(
                "https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved",
                params={"scrIds": scr_id, "count": 100, "offset": offset,
                        "region": "KR", "lang": "en-US"},
...
def _fetch_naver_mobile(api_type, market, market_label, n=60):
    """네이버 모바일 API - 거래량/시총/등락 랭킹"""
    rows = []
    try:
        res = requests.get(
        ...
        for s in res.json().get("stocks", []):
            ...
            code   = s.get("itemCode", "")
            price  = str(s.get("closePrice", "0")).replace(",", "")
            chg    = str(s.get("fluctuationsRatio", "0")).replace(",", "")
            vol    = str(s.get("accumulatedTradingVolume", "0")).replace(",", "")
            marcap = str(s.get("marketValue", "0")).replace(",", "")
            ...
            rows.append({
                ...
                "ChgRate": float(chg    or 0),
                "Volume":  float(vol    or 0),
                "Marcap":  float(marcap or 0),
            })
    except:
        pass
    return rows
...
def get_krx_full_data():
    """
    3단 폴백으로 어떤 환경에서도 데이터 확보
    A: Yahoo predefined screener
    B: 네이버 모바일 API (거래량/시총/등락 통합)
    C: 두 소스 합산
    """
    rows = []

    # ── 플랜 A: Yahoo predefined screener ──
    rows += _fetch_yahoo_predefined("most_actives_kr",  "KOSPI",  300)
    rows += _fetch_yahoo_predefined("most_actives_krq", "KOSDAQ", 300)

    # ── 플랜 B: 네이버 모바일 API (A가 실패했거나 보완용) ──
    naver_rows = []
    for api_type, market, label in [
        ...
        naver_rows += _fetch_naver_mobile(api_type, market, label, n=60)
...
def get_ranking_tables(df_krx):
    empty = {k: pd.DataFrame() for k in ["kospi_cap","kosdaq_cap","kospi_vol","kosdaq_vol","price_up","price_down"]}
    if df_krx.empty:
        return empty
...
with st.sidebar:
    ...
    if dart_key:
        ...
        st.warning("⚠️ DART_API_KEY 미설정\nopendart.fss.or.kr 무료 발급 후 추가 시 PER/PBR이 네이버와 동일해집니다.")
...
# 🔥 KRX 데이터 로드 - 예외가 나도 앱이 죽지 않도록 보호
try:
